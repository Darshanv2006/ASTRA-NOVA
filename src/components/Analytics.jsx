...
function Analytics({ satellites, metrics, collisionRisks }) {
  ...
  const seeds = { '24h': 1, '7d': 7, '30d': 30, '90d': 90 }
  const seed = seeds[timeRange] || 1
  
  const filteredData = satellites
  const activeCount = filteredData.filter(s => s.status === 'active').length
  const warningCount = filteredData.filter(s => s.status === 'warning').length
  const criticalCount = filteredData.filter(s => s.status === 'critical').length

  const avgHealth = (filteredData.reduce((sum, s) => sum + (s.health || 0), 0) / (filteredData.length || 1)).toFixed(1)

  const byType = {
    comm: filteredData.filter(s => s.type === 'comm').length,
    gps: filteredData.filter(s => s.type === 'gps').length,
    earth: filteredData.filter(s => s.type === 'earth').length,
    science: filteredData.filter(s => s.type === 'science').length,
    starlink: filteredData.filter(s => s.type === 'starlink').length,
  }

  const timeRangeConfig = {
    multiplier: seed,
    uptime: (98.5 + seed * 0.08).toFixed(2) + '%',
    uptimeChange: '+' + (seed * 0.01).toFixed(2) + '%',
    anomalies: seed * 3,
    anomalyChange: '+' + (seed * 2),
    risks: Math.max(1, Math.floor((collisionRisks?.length || 0) * (seed / 7))),
    riskChange: '-' + (seed * 3),
    accuracy: (93 + seed * 0.1).toFixed(1) + '%',
    accuracyChange: '+' + (seed * 0.05).toFixed(2) + '%',
    load: (45 + seed * 3) + '%',
    loadChange: '+' + (seed * 2) + '%',
    throughput: (650 + seed * 15) + ' Gbps',
    throughputChange: '+' + (seed * 8) + '%',
  }

  const keyMetrics = [
    { label: 'System Uptime', value: timeRangeConfig.uptime, change: timeRangeConfig.uptimeChange, trend: 'up', icon: Activity, color: 'cyan' },
    { label: 'AI Anomalies Detected', value: timeRangeConfig.anomalies, change: timeRangeConfig.anomalyChange, trend: 'down', icon: Shield, color: 'green' },
    { label: 'Collision Risks (SGP4)', value: timeRangeConfig.risks, change: timeRangeConfig.riskChange, trend: 'down', icon: AlertTriangle, color: 'orange' },
    { label: 'Isolation Forest Acc.', value: timeRangeConfig.accuracy, change: timeRangeConfig.accuracyChange, trend: 'up', icon: Zap, color: 'blue' }
  ]

  const performanceMetrics = [
    { name: 'Data Throughput', current: timeRangeConfig.throughput, previous: timeRangeConfig.throughputChange, change: timeRangeConfig.throughputChange, status: 'good' },
    { name: 'System Load', current: timeRangeConfig.load, previous: '62%', change: timeRangeConfig.loadChange, status: 'warning' },
    { name: 'Average Health', current: `${avgHealth}%`, previous: `${(parseFloat(avgHealth) - 0.5).toFixed(1)}%`, change: '+0.5%', status: parseFloat(avgHealth) > 90 ? 'good' : 'warning' },
    { name: 'Collision Checks', current: `${(1200 + timeRangeConfig.multiplier * 100)}/s`, previous: '1,189/s', change: '+5%', status: 'good' },
    { name: 'Network Latency', current: `${22 + timeRangeConfig.multiplier}ms`, previous: '25ms', change: '-8%', status: 'excellent' },
    { name: 'Storage Usage', current: `${70 + timeRangeConfig.multiplier * 2}%`, previous: '72%', change: '+6%', status: 'warning' },
  ]

  const getFilteredCollisionRisks = useMemo(() => {
    if (!collisionRisks || collisionRisks.length === 0) return []
    return collisionRisks.filter(r => {
      const prob = r.probability ? parseFloat(r.probability) : 50
      return prob > (100 - seed * 5)
    })
  }, [collisionRisks, seed])
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="relative w-48 h-48">
              <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                <motion.circle
                  ...
                  strokeWidth="20"
                  strokeDasharray={`${filteredData.length > 0 ? (activeCount / filteredData.length) * 251.2 : 0} 251.2`}
                  initial={{ strokeDasharray: '0 251.2' }}
                  animate={{ strokeDasharray: `${filteredData.length > 0 ? (activeCount / filteredData.length) * 251.2 : 0} 251.2` }}
                ...
                <motion.circle
                  ...
                  strokeWidth="20"
                  strokeDasharray={`${filteredData.length > 0 ? (warningCount / filteredData.length) * 251.2 : 0} 251.2`}
                  strokeDashoffset={`-${filteredData.length > 0 ? (activeCount / filteredData.length) * 251.2 : 0}`}
                ...
                <motion.circle
                  ...
                  strokeWidth="20"
                  strokeDasharray={`${filteredData.length > 0 ? (criticalCount / filteredData.length) * 251.2 : 0} 251.2`}
                  strokeDashoffset={`-${filteredData.length > 0 ? ((activeCount + warningCount) / filteredData.length) * 251.2 : 0}`}
                  initial={{ strokeDasharray: '0 251.2' }}
                  animate={{
                    strokeDasharray: `${filteredData.length > 0 ? (criticalCount / filteredData.length) * 251.2 : 0} 251.2`,
                    strokeDashoffset: `-${filteredData.length > 0 ? ((activeCount + warningCount) / filteredData.length) * 251.2 : 0}`
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-xs font-mono text-accent-orange">{getFilteredCollisionRisks.length} Active Risks</span>
          </div>

          <div className="space-y-4">
            {getFilteredCollisionRisks.length === 0 ? (
              ...
              getFilteredCollisionRisks.slice(0, 5).map((risk, index) => (
                <motion.div
          ...
          {Object.entries(byType).map(([type, count], index) => (
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-1">
                {filteredData.length > 0 ? ((count / filteredData.length) * 100).toFixed(0) : 0}%
              </p>
            </motion.div>
          ))}
        </div>
