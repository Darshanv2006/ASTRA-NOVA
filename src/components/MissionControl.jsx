...
// SVG Circular Gauge Component - Enhanced with animations
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative">
        ...
        />
        <svg width={size} height={size} viewBox="0 0 80 80" className="relative">
      ...
      <motion.span 
        className="text-[10px] text-gray-500 uppercase tracking-wider mt-1"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
export default function MissionControl({ satellites, selectedSatellite, alerts, metrics, collisionRisks = [], debris = [] }) {
  const [filter, setFilter] = useState('');
  const [localSelectedSat, setLocalSelectedSat] = useState(null);
  const [tab, setTab] = useState('health'); // health | collision | heatmap
  
  // Use external selectedSatellite if available, otherwise use local state
  const activeSat = selectedSatellite || localSelectedSat;
  const setActiveSat = selectedSatellite ? null : setLocalSelectedSat;
  ...
  // Compute selected satellite vitals OR fleet averages
  const displayTemp = useMemo(() => {
    if (activeSat) return activeSat.temperature || 25;
    return (satellites.reduce((s, sat) => s + (sat.temperature || 25), 0) / (satellites.length || 1)).toFixed(1);
  }, [activeSat, satellites]);

  const displayBattery = useMemo(() => {
    if (activeSat) return activeSat.battery || 95;
    return (satellites.reduce((s, sat) => s + (sat.battery || 95), 0) / (satellites.length || 1)).toFixed(1);
  }, [activeSat, satellites]);

  const displaySignal = useMemo(() => {
    if (activeSat) return activeSat.communication?.signalStrength || 90;
    return (satellites.reduce((s, sat) => s + (sat.communication?.signalStrength || 90), 0) / (satellites.length || 1)).toFixed(1);
  }, [activeSat, satellites]);
    ...
    <div className="relative w-full h-[calc(100vh-6rem)] overflow-hidden rounded-2xl border border-white/10 bg-black/60 backdrop-blur-md shadow-2xl flex flex-col">
      ...
      <div className="absolute inset-x-80 inset-y-0 z-0 flex items-center justify-center opacity-80 pointer-events-none">
        <div className="w-[120%] h-[120%]">
          <UltraRealisticEarth satellites={satellites} />
        </div>
      </div>
        ...
        >
          <div className="flex items-center gap-4">
            {[
              ...
              { label: 'DEBRIS', value: debris.length, color: '#f97316' },
            ...
            <div className="glass p-3 rounded-xl border border-blue-500/20 bg-black/60 backdrop-blur-xl h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                {filteredSats.map(sat => (
                  <div 
                    key={sat.id} 
                    onClick={() => setLocalSelectedSat(sat)}
              ...
              {activeSat && (
                ...
                >
                  <div className="glass p-3 rounded-xl border border-accent-cyan/30 bg-black/60 relative">
                    <button 
                      onClick={() => setLocalSelectedSat(null)}
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                      <div className="bg-black/40 rounded p-2">
                        ...
                        <p className="text-white">{Math.round(activeSat.orbital?.altitude || 550)} km</p>
                      </div>
                      <div className="bg-black/40 rounded p-2">
                        <span className="text-gray-500 font-bold uppercase">VEL</span>
                        <p className="text-white">{activeSat.speed?.toFixed(3)} km/s</p>
                      </div>
                      <div className="bg-black/40 rounded p-2">
                        <span className="text-gray-500 font-bold uppercase">TEMP</span>
                        <p className="text-white">{activeSat.temperature?.toFixed(1)}°C</p>
                      </div>
                      <div className="bg-black/40 rounded p-2">
                        <span className="text-gray-500 font-bold uppercase">BATT</span>
                        <p className="text-white">{activeSat.battery?.toFixed(1)}%</p>
                      </div>
                      <div className="bg-black/40 rounded p-2 col-span-2">
                        <span className="text-gray-500 font-bold uppercase">POS</span>
                        <p className="text-white">{activeSat.position?.lat}°N, {activeSat.position?.lng}°E</p>
                      </div>
                    </div>
                    {activeSat.isAnomaly && (
                  ...
                  <motion.div key="health" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                    ...
                    <div className="glass p-4 rounded-xl border border-blue-500/20 bg-black/60">
                      <h4 className="text-[10px] text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                        ...
                        {activeSat ? `Vitals: ${activeSat.name}` : 'Fleet Vitals'}
                      </h4>
                      <div className="grid grid-cols-2 gap-4">
                        <Gauge value={parseFloat(metrics.systemLoad || 0)} label="CPU Load" color="#00d4ff" />
                        <Gauge value={parseFloat(displayTemp)} max={50} label={activeSat ? "Temp" : "Avg Temp"} unit="°C" color="#ef4444" />
                        <Gauge value={parseFloat(displayBattery)} label={activeSat ? "Battery" : "Avg Battery"} color="#22c55e" />
                        <Gauge value={parseFloat(displaySignal)} label={activeSat ? "Signal" : "Avg Signal"} color="#3b82f6" />
                      </div>
                    </div>
