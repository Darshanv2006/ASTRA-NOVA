...
const calculateDistance = (sat1, sat2) => {
  ...
  // Safely extract altitudes, handling malformed telemetry
  const alt1 = sat1?.orbital?.altitude ?? (sat1?.altitude ?? 500)
  const alt2 = sat2?.orbital?.altitude ?? (sat2?.altitude ?? 500)
  const R1 = EARTH_RADIUS + alt1
  const R2 = EARTH_RADIUS + alt2

  const lat1 = (parseFloat(sat1?.position?.lat || 0) * Math.PI) / 180
  const lon1 = (parseFloat(sat1?.position?.lng || 0) * Math.PI) / 180
  const lat2 = (parseFloat(sat2?.position?.lat || 0) * Math.PI) / 180
  const lon2 = (parseFloat(sat2?.position?.lng || 0) * Math.PI) / 180
  ...
  return {
    distance,
    altitudeDiff: Math.abs(alt1 - alt2),
