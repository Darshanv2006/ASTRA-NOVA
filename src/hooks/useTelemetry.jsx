...
// Fetch debris catalog from backend or local file
const fetchDebrisCatalog = async () => {
  try {
    if (USE_BACKEND) {
      const response = await fetch(`${BACKEND_URL}/api/debris`)
      if (!response.ok) throw new Error('Failed to fetch debris')
      const data = await response.json()
      const debris = data.data || data
      return debris.map(d => ({
    ...
    console.warn('Debris catalog fetch failed:', error)
    return []
  }
}
...
// Generate 350 satellites for optimal dashboard performance
const generateSatellites = () => {
  const allSatellites = []
  
  const satelliteTypes = [
    { prefix: 'STARLINK', type: 'starlink', agency: 'SPACEX', baseCount: 150 },
    { prefix: 'ONEWEB', type: 'comm', agency: 'ONEWEB', baseCount: 40 },
    { prefix: 'IRIDIUM', type: 'comm', agency: 'IRIDIUM', baseCount: 30 },
    { prefix: 'GPS', type: 'gps', agency: 'USSF', baseCount: 20 },
    { prefix: 'GLONASS', type: 'gps', agency: 'RUSSIA', baseCount: 15 },
    { prefix: 'GALILEO', type: 'gps', agency: 'ESA', baseCount: 18 },
    { prefix: 'BEIDOU', type: 'gps', agency: 'CHINA', baseCount: 20 },
    { prefix: 'NOAA', type: 'earth', agency: 'NASA', baseCount: 12 },
    { prefix: 'LANDSAT', type: 'earth', agency: 'NASA', baseCount: 8 },
    { prefix: 'SENTINEL', type: 'earth', agency: 'ESA', baseCount: 10 },
    { prefix: 'ISS', type: 'science', agency: 'NASA', baseCount: 1 },
    { prefix: 'HUBBLE', type: 'science', agency: 'NASA', baseCount: 1 },
    { prefix: 'GOES', type: 'geo', agency: 'NASA', baseCount: 5 },
    { prefix: 'METEOSAT', type: 'geo', agency: 'ESA', baseCount: 3 },
    { prefix: 'INTELSAT', type: 'comm', agency: 'OTHER', baseCount: 20 },
    { prefix: 'SES', type: 'comm', agency: 'OTHER', baseCount: 15 },
    { prefix: 'EUTELSAT', type: 'comm', agency: 'OTHER', baseCount: 10 },
    { prefix: 'OTHER', type: 'other', agency: 'OTHER', baseCount: 12 }
  ]
  ...
  satelliteTypes.forEach(({ prefix, type, agency, baseCount }) => {
    const count = Math.min(baseCount, MAX_SATELLITES - allSatellites.length)
    
    for (let i = 0; i < count && allSatellites.length < MAX_SATELLITES; i++) {
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      setMetrics({
        ...
        debrisCount: (data || []).length // Not used yet, but could show
      })
